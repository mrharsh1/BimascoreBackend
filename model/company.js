...
db.connect((err) => {
  (err)? console.warn('error bimascore company') : console.warn('connected bimascore company')
})
...
module.exports = {
  ...
  getTbl: function (where='1=1', select='*', order='id desc'){
    return new Promise((resolve) => {
      db.query('select '+select+' from echron_tb_companies where '+where+' order by '+order, (err, result) => {
        if(err) {

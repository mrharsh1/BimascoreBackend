...
db.connect((err) => {
  (err)? console.warn('error bimascore features') : console.warn('connected bimascore features')
})
...
module.exports = {
  ...
  getTbl: function (where='1=1', select='*', order='id desc'){
    return new Promise((resolve) => {
      db.query('select '+select+' from  echron_tb_policy_overview where '+where+' order by '+order, (err, result) => {
        if(err) {

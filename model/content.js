...
db.connect((err) => {
  (err)? console.warn('error bimascore Contents') : console.warn('connect bimascore Contents')
})
...
module.exports = {
  ...
  getTbl: function (where='1=1', select='*', order='id desc'){
    return new Promise((resolve) => {
      db.query('select '+select+' from  echron_tb_contents where '+where+' order by '+order, (err, result) => {
        if(err) {
